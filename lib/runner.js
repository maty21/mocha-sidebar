...
Runner.prototype._runMocha = function(testFiles, grep, logMessages) {
  ...
  return MochaShim.runAsyncTests(dedupeStrings(tests), grep, logMessages).then(
    result => {
...
Runner.prototype._runMochaSingleTest = function(testFiles, grep, logMessages) {
  return MochaShim.runTests(dedupeStrings(testFiles), grep, logMessages).then(
    result => {
      lastRunResult = result;
...
Runner.prototype.runFailed = async function() {
  ...
  if (!failed.length) {
    ...
    const res = await this.runAsyncTests(dedupeStrings(failed.map(test => test.file)), `^${failed.map(test => `(${escapeRegExp(test.fullName)})`).join("|")}$`);
    mochaProviderRef.hookResultsFromCommands(res);
  }
};
...
Runner.prototype.runLastSet = async function() {
  ...
  if (!set.length) {
    ...
    const res = this.runAsyncTests(dedupeStrings(set.map(test => test.file)), `^${set.map(test => `(${escapeRegExp(test.fullName)})`).join("|")}$`);
  }
