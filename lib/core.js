...
class Core extends EventEmitter {
  ...
  runTestAtCursor(fileName, line) {
    const testItem = this.enrichedTests.find(test => test.file === fileName && line === test.meta[0].number);
    if (!testItem) {
      const describerItem = this.enrichedDescribers.find(test => test.file === fileName && line === test.meta[0].number);
      if (!describerItem) {
