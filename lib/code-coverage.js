...
class codeCoverage {
  ...
  updateDecorationByFile(editor){
    if(this._toggleCoverage){
      if( this.getResultByPath(vscode.window.activeTextEditor.document.fileName)){
        this.getResultByPath(vscode.window.activeTextEditor.document.fileName).lines.details.forEach(l=>{
            const ratio = getRatio(l.hit);
            setDecoration(ratio,l.line,l.hit,vscode.window.activeTextEditor.document.fileName);
          })
