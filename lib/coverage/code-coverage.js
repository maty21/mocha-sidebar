...
class codeCoverage {
  constructor(workDir = vscode.workspace.rootPath) {
    ...
    //   this.nodeModulesPath = getNodeModules();
  ...
  _spawnCoverage() {
    return new Promise(async (resolve, reject) => {
      ...
      const cp = spawn(
        path,
        // [this._pathToNyc, "--all", "--reporter=html", "--reporter=text", "--reporter=lcov", "/Users/maty21/mocha-sidebar/test/workspace/node_modules/mocha/bin/mocha", "test/**/*.js", "--exit"],
        [this._pathToNyc, ...this._cmd],
        {
          cwd: this._workDir
          // stdio: 'inherit',
        }
